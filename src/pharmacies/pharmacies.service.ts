...
export class PharmaciesService {
  async getPharmacies(payload: GetPharmacyDto) {
    const url = `https://www.pagesjaunes.ci/pharmacies-de-garde-${payload.commune}/`;
  ...
  private scrapePharmacies(html: string) {
    ...
    const FiltrePharmacies = listePharmacies.filter((item) =>
      item.link.includes('Pharmacie '),
    );
